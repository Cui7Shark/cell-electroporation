...
Th=10;
alpha = 1e9;
dudt=zeros(a+a1+a2+a3,1);
Vapp=  0.*(t<0)+(10e6.*t).*(t<1e-6 & t>=0) + 10.*(t>=1e-6 & t<99e-6) + (-10e6.*t+1e3).*(t>=99e-6 & t<=100e-6)+ 0.*(t>100e-6);
t
%%
for i = 1:(a+a1+a2+a3)
    ...
    if size(col,1)==0 && i<=a                                              %%非膜上节点的电流
        ...
        dudt(i) = 6.00e13*Res(i,:)*u(1:a) + 6.00e13*Rus(i)*Vapp;                           %求电流
...
%%计算跨膜电流
        %细胞膜内节点
        if col==1 
            dudt(i) = 6.00e13*Res(i,:)*u(1:a) - 6.00e13*Gp(u)*u(row+a)*Vm(u)*K(row,3)*Th*1e-12;      %1e-6*1e-6*;
        %细胞膜外-节点
        elseif col==2 
            dudt(i) =6.00e13* Res(i,:)*u(1:a) + 6.00e13*Gp(u)*u(row+a)*Vm(u)*K(row,3)*Th*1e-12;      %1e-6*1e-6;
        end
